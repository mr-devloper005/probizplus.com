...
const inputClass = 'h-12 w-full rounded-xl border border-[var(--editable-border)] bg-[#f8fafc] px-4 text-sm font-semibold text-[var(--slot4-page-text)] outline-none transition placeholder:text-[#7b849f] focus:border-[var(--slot4-accent)] focus:bg-white'
const buttonClass = 'inline-flex h-12 w-full items-center justify-center rounded-xl bg-[#1455cc] px-6 text-sm font-bold uppercase tracking-[0.12em] text-white transition hover:-translate-y-0.5 hover:brightness-110 active:scale-[0.98] disabled:cursor-not-allowed disabled:opacity-60'
...
export function EditableLocalLoginForm() {
  ...
  return (
    <form className="mt-6 grid gap-4" onSubmit={submit}>
      ...
      {message ? <p className={`rounded-xl px-4 py-3 text-sm font-semibold ${status === 'success' ? 'bg-emerald-50 text-emerald-700' : 'bg-[var(--slot4-accent-soft)] text-[var(--slot4-accent)]'}`}>{message}</p> : null}
