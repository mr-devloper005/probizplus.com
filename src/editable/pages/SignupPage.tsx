...
const onboardingPoints = [
  'Create your account once for faster future publishing access.',
  'Keep core details saved in one clear workspace.',
  'Start with the same visual rhythm used throughout the site.',
]
        ...
        <section className="mx-auto max-w-[var(--editable-container)] px-4 py-10 sm:px-6 lg:px-8">
          <div className="grid gap-8 lg:grid-cols-[1.18fr_0.82fr]">
            <div className="rounded-[1.8rem] border border-[var(--editable-border)] bg-white p-6 shadow-[0_22px_44px_rgba(18,13,55,0.08)] sm:p-8">
              <p className="text-xs font-bold uppercase tracking-[0.16em] text-[#1455cc]">{pagesContent.auth.signup.formTitle}</p>
              ...
              <div className="mt-6 flex flex-wrap items-center justify-between gap-3 rounded-xl bg-[#f8fafc] px-4 py-4">
                <p className="text-sm text-[var(--slot4-muted-text)]">
                  ...
                  <Link href="/login" className="font-bold text-[var(--slot4-accent)] underline-offset-4 hover:underline">
                    {pagesContent.auth.signup.loginCta}
                  </Link>
                </p>
                <Link href="/login" className="inline-flex items-center gap-2 text-sm font-bold uppercase tracking-[0.12em] text-[#1455cc]">
